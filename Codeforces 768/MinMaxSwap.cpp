...
{ // Snippets Variables
    ...
    int checkmaxb = max(b, n);

    // debug(checkmaxa);
    // debug(checkmaxb);

    if (a[checkmaxa] < b[checkmaxb])
    {
    ...
    while (true)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (b[i] <= a[tempMaxA] && b[i] > b[justMaxB])
            {
                justMaxB = i;
            }
        }
        // debug(justMaxB);
        if (a[justMaxB] < b[justMaxB])
        {
            swap(a[justMaxB], b[justMaxB]);
            // debug(a[justMaxB]);
            tempMaxA = justMaxB;
        }
        else
