...
{ // Snippets Variables
    ...
    cin >> n;
    int ActualNoOfDigit = countDigit(n);
    ...
    int realNo = n;
    int diffDigit = 0, ansLastDigit = 0, noLastDigit = 0;
    int rightAns = ans;
    while (ans != 0)
    {
        ansLastDigit = ans % 10;
        noLastDigit = n % 10;
        if (ansLastDigit != noLastDigit)
        {
            diffDigit++;
        }
        n = n / 10;
    ...
    if ((diffDigit > 1 || countDigit(rightAns) == 1) || countDigit(realNo) > countDigit(rightAns))
    {
        rightAns = rightAns + 7;
    }
